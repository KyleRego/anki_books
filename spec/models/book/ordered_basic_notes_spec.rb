...
RSpec.describe Book, "#ordered_basic_notes" do
  ...
  it "returns the notes ordered by ordinal position when the book has one article" do
    ...
    expect(book.ordered_basic_notes).to eq article.ordered_basic_notes
  end
  ...
  it "returns the notes ordered by ordinal positions of the articles when the book has many articles" do
    ...
    expect(book.ordered_basic_notes.pluck(:article_id)).to eq book.ordered_articles.pluck(:id)
  end
  ...
  it "returns first the basic notes of the first article in order" do
    ...
    expect(book.ordered_basic_notes.first(3).pluck(:id)).to eq book.articles.find_by(ordinal_position: 0).ordered_basic_notes.pluck(:id)
  end
end
