...
RSpec.describe UpdateUserAnkiPackagesJob do
  ...
  before do
    allow(CreateUserAnkiDeck).to receive(:perform).with(user:).and_return(updated_package_fixture_path)
    mock_job = class_double(DeleteAnkiDeckJob)
    allow(DeleteAnkiDeckJob).to receive(:set).with(wait: 3.minutes).and_return(mock_job)
